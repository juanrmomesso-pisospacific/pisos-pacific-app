...
- **UX Tanda 2a — confirmaciones (17/6):** `ConfirmProvider`/`useConfirm()` (`components/ui/confirm.tsx`, modal radix-dialog, montado en App.tsx) reemplaza los `window.confirm`. Aplicado a las acciones peligrosas: **Cancelar venta**, **Finalizar (descontar stock)**, **Rechazar cotización**, **Renovar vigencia**, **Unificar leads** (muestra a qué lead fusiona), **Borrar regla** de clasificación. **(2b)** Compartir cotización ya NO usa `window.prompt`/`alert`: `ShareQuoteSheet` (RowActions) = panel con mensaje editable (textarea), botones WhatsApp/Email con feedback inline, y link público copiable. (Tanda 2 sigue: estados en español, estados de carga/error.)
