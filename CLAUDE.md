...
- **SEGURIDAD P0 (backend):** (a) webhooks de Meta (`/api/whatsapp|instagram/webhook`) NO validan `X-Hub-Signature-256` → cualquiera con la URL puede inyectar leads/mensajes y, si falsea un `from` en `CASH_ALLOWLIST`, **meter egresos en Caja General** o disparar `cancelar`. Verificar HMAC con app secret. (b) `/api/payment-links/:id/simulate-paid` exento de auth **registra cobros reales** (id enumerable) → restringir a links `mode:'mock'`. (c) **No hay control de rol**: cualquier sesión (incl. vendor) puede POST/PATCH/DELETE todo el CRUD (cashflow, ventas, reglas) y no se filtran ventas por vendedor → falta `requireAdmin` + filtrado por `seller_name`. OJO: aplicar (c) cambia qué ve cada usuario, avisar al dueño.
