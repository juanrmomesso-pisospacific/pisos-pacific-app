...
- **MP — arreglos base (revisión 15/6):** enriquecimiento matchea por **id de operación** (exacto, `mp_op_id`) con fallback a fecha±3+monto (antes solo fuzzy → duplicados/colisiones); egresos sin nombre van a categoría **"Sin clasificar"** con `expense_type` definido (antes `null` → no contaban en P&L); **TC Blue en vivo** compartido (`import/fx.mjs`, `getBlueRate/lastBlue`) en sync MP + extractos + bot de gastos (antes 1400 fijo). **Verificado (con datos reales): la API de MP NO da nombres de contraparte** (`/v1/account/bank_report` 404 con token de app; settlement solo trae códigos internos). Los nombres siguen viniendo del export manual mensual. Pendiente: S2 (reglas de clasificación editables + que aprenden).
