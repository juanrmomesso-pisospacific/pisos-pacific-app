...
- **Compartir presupuesto (B):** acción en la cotización → "Compartir por WhatsApp" (sube el PDF como documento por Cloud API al teléfono del cliente, `sendWhatsAppDocument` en meta.mjs, y lo refleja en la conversación) y "Copiar link (Instagram)" (link público al PDF `GET /p/q/:id/:token`, sin login, token aleatorio `quote.share_token`). Endpoint `POST /api/quotes/:id/share`.
