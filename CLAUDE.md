...
- ✅ Webhooks de Meta validan `X-Hub-Signature-256` (HMAC con `META_APP_SECRET`). Si el env NO está cargado, deja pasar con warning (no romper el webhook hasta configurarlo). **PENDIENTE: cargar `META_APP_SECRET` en Render** (App Dashboard Meta → Configuración → Básica → Clave secreta de la app) y redeployar (`POST /deploys`) para que la verificación quede activa.
...
- **SEGURIDAD (resto):** filtrar GET `/api/sales` y `/api/quotes` por `seller_name` para no-admin (necesita que el front maneje datos parciales: dashboard/reportes) y **ocultar la navegación financiera a vendedores** en el front (hoy el backend ya los bloquea con 403 pero la UI se los muestra). Cargar `META_APP_SECRET` (arriba).
- **CORRECTITUD P1:** inbound de Meta es fire-and-forget con catch que solo loguea (mensajes se pierden silenciosos si falla, Meta ya recibió 200); sin dedup de inbound por `m.id` ni serialización → race en el bot de efectivo (doble gasto); `fetch` externos sin timeout (Graph/Gmail/MP pueden colgar requests y el scheduler); status de entrega WhatsApp (delivered/read) nunca se procesa; `share` matchea conversación por sufijo de 8 dígitos (puede cruzar chats); reconciliación de cobros por `sale_ref` vs `quote_number` puede no imputar.
