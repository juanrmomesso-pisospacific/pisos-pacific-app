...
| **WhatsApp** | Código listo en `meta.mjs`; PENDIENTE conectar con un número NUEVO dedicado (decisión del dueño). |
...
_Actualizado: 2026-06-11. HEAD = `ddebf11` en `main` y `feature/data-import-cashflow` (ambos pusheados). Producción deployada y verificada (healthz 200, login 200). Working tree limpio salvo `pisos-pacific-app/` (carpeta anidada espuria, sin trackear — candidata a borrar)._
...
**Próximos pasos**
1. **WhatsApp** con número nuevo dedicado (código listo; recordar lección "modo Live").
2. **Refresh automático del token de Instagram** (vence ~60 días; hoy manual).
