...
- **Fase A (galería) HECHA**: página **Galería** (`/galeria`, `GaleriaPage`) navega el Drive de marca (carpeta raíz `1GttGPDMj120WiYPgCimclfsLFwopV107`, en info@pisospacific.com). Drive **privado** → la app lo lee con scope `drive.readonly` (sumado a la conexión Google "pacific") y sirve las fotos por **proxy con caché** (`integrations/drive.mjs`; endpoints `/api/drive/status|folder|file/:id`; caché en `UPLOAD_DIR/drive-cache`). **Requiere que el dueño re-autorice** Google una vez (`/api/integrations/google/connect?account=pacific` ahora pide también Drive) — hasta entonces la galería muestra "Conectar Google Drive". `GDRIVE_REFRESH_TOKEN` = token pacific re-autorizado.
