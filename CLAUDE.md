...
- ✅ Webhooks de Meta validan `X-Hub-Signature-256`. **WhatsApp: enforce** con `META_APP_SECRET` (sin firma válida → 403, confirmado). **Instagram firma con SU PROPIO secret** (gotcha Meta confirmado por logs: con `META_APP_SECRET` da "firma INVÁLIDA") → `metaSignatureOk(req, channel)` usa `IG_APP_SECRET` para IG. **Estado: IG en log-only hasta cargar `IG_APP_SECRET` en Render** (apenas se carga, IG pasa a enforce automáticamente; el código ya está deployado). El Instagram App Secret está en developers.facebook.com → app Pacific → producto Instagram → configuración de la API (NO es el de Settings→Basic).
