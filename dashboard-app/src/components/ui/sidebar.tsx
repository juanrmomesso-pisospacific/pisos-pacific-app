...
export function useSidebar() {
  const ctx = React.useContext(SidebarContextProvider)
...
export function SidebarProvider({
  ...
  const setOpen = React.useCallback((value: boolean | ((v: boolean) => boolean)) => {
    const v = typeof value === "function" ? (value as any)(open) : value
  ...
  const value: SidebarContext = { state, open, setOpen: setOpen as any, openMobile, setOpenMobile, isMobile, toggleSidebar }
...
export function Sidebar({
  ...
  if (collapsible === "none") {
    return <div className={cn("flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground", className)} {...props}>{children}</div>
  }
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>Navegación lateral</SheetDescription>
          </SheetHeader>
...
export function SidebarGroupLabel({ className, asChild = false, ...props }: React.HTMLAttributes<HTMLDivElement> & { asChild?: boolean }) {
...
export function SidebarGroupAction({ className, asChild = false, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean }) {
  const Comp = asChild ? Slot : "button"
  return <Comp data-sidebar="group-action" className={cn("absolute right-3 top-3.5 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2", className)} {...props} />
}
...
>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
  ...
  const button = (
    <Comp ref={ref} data-sidebar="menu-button" data-active={isActive} className={cn(sidebarMenuButtonVariants({ variant, size }), className)} {...props} />
  )
...
export function SidebarMenuAction({ className, asChild = false, showOnHover = false, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { asChild?: boolean; showOnHover?: boolean }) {
  const Comp = asChild ? Slot : "button"
  return (
    <Comp
...
export function SidebarMenuBadge({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div data-sidebar="menu-badge" className={cn("absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium tabular-nums text-sidebar-foreground select-none pointer-events-none", "peer-hover/menu-button:text-sidebar-accent-foreground peer-data-[active=true]/menu-button:text-sidebar-accent-foreground", "peer-data-[size=sm]/menu-button:top-1 peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5", "group-data-[collapsible=icon]:hidden", className)} {...props} />
}

export function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.HTMLAttributes<HTMLDivElement> & { showIcon?: boolean }) {
  ...
  return (
    <div data-sidebar="menu-skeleton" className={cn("rounded-md h-8 flex gap-2 px-2 items-center", className)} {...props}>
      ...
      <Skeleton className="h-4 flex-1 max-w-(--skeleton-width)" data-sidebar="menu-skeleton-text" style={{ ["--skeleton-width" as any]: width }} />
    </div>
  )
}
...
export function SidebarMenuSubButton({ asChild = false, size = "md", isActive, className, ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement> & { asChild?: boolean; size?: "sm" | "md"; isActive?: boolean }) {
